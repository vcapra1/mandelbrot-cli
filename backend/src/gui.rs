...
fn handle_connection(stream: TcpStream) -> bool {
    ...
    let valid_files: HashMap<_, _> = vec![("/style.css", "text/css"), ("/sample.png", "image/png")]
        .into_iter()
    ...
    let (exit, bytes_sent) = if request.method() == "GET" {
        if valid_files.contains_key(&request.target().path) {
            ...
            (
                false,
                request
                    .respond_with_file("200 OK", HashMap::new(), filename, filetype)
                    .unwrap_or(0),
            )
            ...
            (
                false,
                request
                    .respond("404 Not Found", HashMap::new(), Vec::new())
                    .unwrap_or(0),
            )
        }
        ...
        (
            false,
            request
                .respond("405 Method Not Allowed", HashMap::new(), Vec::new())
                .unwrap_or(0),
        )
    };
